...
void SnomaskManager::WriteToSnoMaskRemote(Snomask &mask, const std::string &text)
{
	mask.SendMessage(text, true);
}

void SnomaskManager::WriteToSnoMask(Snomask &mask, const std::string &text)
{
	mask.SendMessage(text, false);
}

void SnomaskManager::WriteGlobalSno(Snomask &mask, const std::string& text)
{
	WriteToSnoMaskRemote(mask, text);
	ServerInstance->PI->SendSNONotice(mask, text);
}

void SnomaskManager::WriteToSnoMaskRemote(Snomask &mask, const char* text, ...)
{
	std::string textbuffer;
	VAFORMAT(textbuffer, text, text);
	WriteToSnoMaskRemote(mask, textbuffer);
}

void SnomaskManager::WriteToSnoMask(Snomask &mask, const char* text, ...)
{
	std::string textbuffer;
	VAFORMAT(textbuffer, text, text);
	WriteToSnoMask(mask, textbuffer);
}

void SnomaskManager::WriteGlobalSno(Snomask &mask, const char* text, ...)
{
	std::string textbuffer;
	VAFORMAT(textbuffer, text, text);
	WriteGlobalSno(mask, textbuffer);
}
