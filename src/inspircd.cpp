...
void InspIRCd::Run()
{
	...
	while (true)
	{
		...
		if (TIME.tv_sec != OLDTIME)
		{
			...
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::debug, "\002EH?!\002 -- Time is flowing BACKWARDS in this dimension! Clock drifted backwards %lu secs.", (unsigned long)OLDTIME-TIME.tv_sec);
			}
			else if (TIME.tv_sec > OLDTIME + 2)
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::debug, "\002EH?!\002 -- Time is jumping FORWARDS! Clock skipped %lu secs.", (unsigned long)TIME.tv_sec - OLDTIME);
			}
