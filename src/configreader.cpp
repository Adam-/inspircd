...
void ServerConfig::Apply(ServerConfig* old, const std::string &useruid)
{
	...
	while (errstr.good())
	{
		...
		// Also tell opers
		SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, line);
	}
	...
	SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "*** Successfully rehashed server.");
}
...
void ServerConfig::ApplyModules(User* user)
{
	...
	for (ModuleManager::ModuleMap::iterator i = removed_modules.begin(); i != removed_modules.end(); ++i)
	{
		...
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "*** REHASH UNLOADED MODULE: %s", modname.c_str());

			if (user)
				user->WriteNumeric(RPL_UNLOADEDMODULE, "%s :Module %s successfully unloaded.", modname.c_str(), modname.c_str());
			else
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Module %s successfully unloaded.", modname.c_str());
		}
		else
		{
			if (user)
				user->WriteNumeric(ERR_CANTUNLOADMODULE, "%s :Failed to unload module %s: %s", modname.c_str(), modname.c_str(), ServerInstance->Modules->LastError().c_str());
				 ...
				 SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Failed to unload module %s: %s", modname.c_str(), ServerInstance->Modules->LastError().c_str());
		}
	}
	...
	for (std::vector<std::string>::iterator adding = added_modules.begin(); adding != added_modules.end(); adding++)
	{
		...
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "*** REHASH LOADED MODULE: %s",adding->c_str());
			if (user)
				user->WriteNumeric(RPL_LOADEDMODULE, "%s :Module %s successfully loaded.", adding->c_str(), adding->c_str());
			else
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Module %s successfully loaded.", adding->c_str());
		}
		else
		{
			if (user)
				...
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Failed to load module %s: %s", adding->c_str(), ServerInstance->Modules->LastError().c_str());
		}
	}
}
