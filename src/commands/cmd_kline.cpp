...
CmdResult CommandKline::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	if (parameters.size() >= 3)
	{
		...
		if (ServerInstance->XLines->AddLine(kl,user))
		{
			if (!duration)
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added permanent K-line for %s: %s",user->nick.c_str(),target.c_str(), parameters[2].c_str());
			}
			else
			{
				time_t c_requires_crap = duration + ServerInstance->Time();
				std::string timestr = InspIRCd::TimeString(c_requires_crap);
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added timed K-line for %s, expires on %s: %s",user->nick.c_str(),target.c_str(),
	...
	else
	{
		...
		{
			SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s removed K-line on %s",user->nick.c_str(),target.c_str());
		}
		else
		{
