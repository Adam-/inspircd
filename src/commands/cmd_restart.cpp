...
CmdResult CommandRestart::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	{
		SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "RESTART command from %s, restarting server.", user->GetFullRealHost().c_str());
		...
		SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Failed RESTART - could not execute '%s' (%s)",
			ServerInstance->Config->cmdline.argv[0], strerror(errno));
	}
	else
	{
		SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Failed RESTART Command from %s.", user->GetFullRealHost().c_str());
	}
	return CMD_FAILURE;
}
