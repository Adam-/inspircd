...
CmdResult CommandKill::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	if ((u) && (!IS_SERVER(u)))
	{
		...
		{
			// remote kill
			SnomaskManager::WriteToSnoMaskRemote(SnomaskManager::kill, "Remote kill by %s: %s (%s)", user->nick.c_str(), u->GetFullRealHost().c_str(), parameters[1].c_str());
		...
		else
		{
			 ...
			 */
			if (IS_LOCAL(user))
				SnomaskManager::WriteGlobalSno(SnomaskManager::kill,"Local Kill by %s: %s (%s)", user->nick.c_str(), u->GetFullRealHost().c_str(), parameters[1].c_str());
			else
				SnomaskManager::WriteToSnoMask(SnomaskManager::kill,"Local Kill by %s: %s (%s)", user->nick.c_str(), u->GetFullRealHost().c_str(), parameters[1].c_str());
