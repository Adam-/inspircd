...
CmdResult CommandZline::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	if (parameters.size() >= 3)
	{
		...
		if (ServerInstance->XLines->AddLine(zl,user))
		{
			if (!duration)
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added permanent Z-line for %s: %s", user->nick.c_str(), ipaddr, parameters[2].c_str());
			}
			else
			{
				time_t c_requires_crap = duration + ServerInstance->Time();
				std::string timestr = InspIRCd::TimeString(c_requires_crap);
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added timed Z-line for %s, expires on %s: %s",user->nick.c_str(),ipaddr,
	...
	else
	{
		...
		{
			SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s removed Z-line on %s",user->nick.c_str(),target.c_str());
		}
		else
		{
