...
CmdResult CommandQline::Handle (const std::vector<std::string>& parameters, User *user)
{
	if (parameters.size() >= 3)
	{
		...
		if (ServerInstance->XLines->AddLine(ql,user))
		{
			if (!duration)
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added permanent Q-line for %s: %s",user->nick.c_str(), parameters[0].c_str(), parameters[2].c_str());
			}
			else
			{
				time_t c_requires_crap = duration + ServerInstance->Time();
				std::string timestr = InspIRCd::TimeString(c_requires_crap);
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added timed Q-line for %s, expires on %s: %s",user->nick.c_str(),parameters[0].c_str(),
	...
	else
	{
		...
		{
			SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s removed Q-line on %s",user->nick.c_str(),parameters[0].c_str());
		}
		else
		{
