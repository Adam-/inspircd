 ...
 public:
	...
	CmdResult Handle (const std::vector<std::string>& parameters, User *user)
	{
		...
		if (target && IS_LOCAL(target))
		{
			...
			{
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, oldnick+" used SANICK to change "+newnick+" to "+parameters[1]);
			}
			else
			{
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, oldnick+" failed SANICK (from "+newnick+" to "+parameters[1]+")");
			}
		}
