 ...
 public:
	...
	CmdResult Handle (const std::vector<std::string> &parameters, User *user)
	{
		...
		if (InspIRCd::Match(ServerInstance->Config->ServerName.c_str(), servername))
		{
			...
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "NEW MODULE '%s' GLOBALLY LOADED BY '%s'",parameters[0].c_str(), user->nick.c_str());
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "MODULE '%s' GLOBAL LOAD BY '%s' (not loaded here)",parameters[0].c_str(), user->nick.c_str());
	...
	CmdResult Handle (const std::vector<std::string> &parameters, User *user)
	{
		...
		if (InspIRCd::Match(ServerInstance->Config->ServerName.c_str(), servername))
		{
			...
			if (m)
			{
				...
				{
					SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "MODULE '%s' GLOBALLY UNLOADED BY '%s'",parameters[0].c_str(), user->nick.c_str());
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "MODULE '%s' GLOBAL UNLOAD BY '%s' (not unloaded here)",parameters[0].c_str(), user->nick.c_str());
...
class GReloadModuleWorker : public HandlerBase1<void, bool>
{
 public:
	...
	{
		SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "MODULE '%s' GLOBALLY RELOADED BY '%s'%s", name.c_str(), nick.c_str(), result ? "" : " (failed here)");
 ...
 public:
	...
	CmdResult Handle(const std::vector<std::string> &parameters, User *user)
	{
		...
		if (InspIRCd::Match(ServerInstance->Config->ServerName.c_str(), servername))
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "MODULE '%s' GLOBAL RELOAD BY '%s' (not reloaded here)",parameters[0].c_str(), user->nick.c_str());
