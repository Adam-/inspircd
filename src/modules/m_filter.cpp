...
CmdResult CommandFilter::Handle(const std::vector<std::string> &parameters, User *user)
{
	if (parameters.size() == 1)
	{
		...
		if (static_cast<ModuleFilter *>(me)->DeleteFilter(parameters[0]))
		{
			...
			if (IS_LOCAL(user))
				SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "FILTER: "+user->nick+" removed filter '"+parameters[0]+"'");
			else
				SnomaskManager::WriteToSnoMaskRemote(SnomaskManager::announcement, "FILTER: "+user->nick+" removed filter '"+parameters[0]+"'");
			return CMD_SUCCESS;
		}
		else
		{
	...
	else
	{
		...
		if (parameters.size() >= 4)
		{
			...
			if (result.first)
			{
				...
				if (IS_LOCAL(user))
					SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "FILTER: "+user->nick+" added filter '"+freeform+"', type '"+parameters[1]+"', "+(duration ? "duration "+parameters[3]+", " : "")+"flags '"+flags+"', reason: "+parameters[reasonindex]);
				else
					SnomaskManager::WriteToSnoMaskRemote(SnomaskManager::announcement, "FILTER: "+user->nick+" added filter '"+freeform+"', type '"+parameters[1]+"', "+(duration ? "duration "+parameters[3]+", " : "")+"flags '"+flags+"', reason: "+parameters[reasonindex]);
...
ModResult ModuleFilter::OnUserPreMessage(User* user, void* dest, int target_type, std::string& text, char status, CUList& exempt_list, MessageType msgtype)
{
	...
	if (f)
	{
		...
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "FILTER: "+user->nick+" had their message filtered, target was "+target+": "+f->reason);
...
void ModuleFilter::ReadConfig(ConfigStatus& status)
{
	...
	{
		if (newrxengine.empty())
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "WARNING: No regex engine loaded - Filter functionality disabled until this is corrected.");
		else
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "WARNING: Regex engine '%s' is not loaded - Filter functionality disabled until this is corrected.", newrxengine.c_str());
	...
	{
		SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Dumping all filters due to regex engine change");
		FreeFilters();
	}
