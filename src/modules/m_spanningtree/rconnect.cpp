...
CmdResult CommandRConnect::Handle (const std::vector<std::string>& parameters, User *user)
{
	...
	if (InspIRCd::Match(ServerInstance->Config->ServerName,parameters[0]))
	{
		...
		SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Remote CONNECT from %s matching \002%s\002, connecting server \002%s\002",user->nick.c_str(),parameters[0].c_str(),parameters[1].c_str());
