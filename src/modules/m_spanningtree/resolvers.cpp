...
void ServernameResolver::OnLookupComplete(const DNS::Query *r)
{
	...
	if (!CheckDupe) /* Check that nobody tried to connect it successfully while we were resolving */
	{
		...
		else
		{
			...
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "CONNECT: Error connecting \002%s\002: %s.",
...
void ServernameResolver::OnError(const DNS::Query *r)
{
	...
	SnomaskManager::WriteToSnoMask(Utils->Creator->link, "CONNECT: Error connecting \002%s\002: Unable to resolve hostname - %s", MyLink->Name.c_str(), this->manager->GetErrorStr(r->error).c_str());
