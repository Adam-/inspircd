...
void TreeSocket::OnConnected()
{
	if (this->LinkState == CONNECTING)
	{
		...
		SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Connection to \2%s\2[%s] started.", linkID.c_str(),
...
{
	SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Connection to '\002%s\002' failed with error: %s",
...
void TreeSocket::Squit(TreeServer* Current, const std::string &reason)
{
	...
	if (!Current->IsRoot())
	{
		...
		if (Current->IsLocal())
		{
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Server \002"+Current->GetName()+"\002 split: "+reason);
			LocalSquit = true;
		...
		{
			SnomaskManager::WriteGlobalSno(Utils->Creator->link, "Server \002"+Current->GetName()+"\002 split from server \002"+Current->GetParent()->GetName()+"\002 with reason: "+reason);
		}
		...
		if (LocalSquit)
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Netsplit complete, lost \002%d\002 user%s on \002%d\002 server%s.",
				num_lost_users, num_lost_users != 1 ? "s" : "", num_lost_servers, num_lost_servers != 1 ? "s" : "");
		else
			SnomaskManager::WriteToSnoMaskRemote(Utils->Creator->link, "Netsplit complete, lost \002%d\002 user%s on \002%d\002 server%s.",
				num_lost_users, num_lost_users != 1 ? "s" : "", num_lost_servers, num_lost_servers != 1 ? "s" : "");
		Current->Tidy();
