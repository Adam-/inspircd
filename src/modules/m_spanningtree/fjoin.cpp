...
CmdResult CommandFJoin::Handle(User* srcuser, std::vector<std::string>& params)
{
	...
	if (!TS)
	{
		...
		SnomaskManager::WriteToSnoMask(SnomaskManager::debug, "WARNING: The server %s is sending FJOIN with a TS of zero. Total craq. Command was dropped.", srcuser->server->GetName().c_str());
		return CMD_INVALID;
	}
	...
	else
	{
		...
		if (TS != ourTS)
		{
			SnomaskManager::WriteToSnoMask(SnomaskManager::debug, "Merge FJOIN received for %s, ourTS: %lu, TS: %lu, difference: %lu",
...
bool CommandFJoin::ProcessModeUUIDPair(const std::string& item, TreeSocket* src_socket, Channel* chan, irc::modestacker* modestack)
{
	...
	if ((modestack) && (comma > 0) && (comma != std::string::npos))
	{
		...
		for (std::string::const_iterator i = item.begin(); i != commait; ++i)
		{
			...
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::debug, "Unrecognised mode '%c' for a user in FJOIN, dropping link", *i);
				return false;
			}
