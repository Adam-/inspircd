		...
		case WAIT_AUTH_2:
			...
			else if (command == "BURST")
			{
				if (params.size())
				{
					...
					{
						SnomaskManager::WriteGlobalSno(Utils->Creator->link, "\2ERROR\2: Your clocks are out by %d seconds (this is more than five minutes). Link aborted, \2PLEASE SYNC YOUR CLOCKS!\2",abs((long)delta));
					...
					{
						SnomaskManager::WriteGlobalSno(Utils->Creator->link, "\2WARNING\2: Your clocks are out by %d seconds. Please consider synching your clocks.", abs((long)delta));
					}
				}
...
{
	SnomaskManager::WriteGlobalSno(Utils->Creator->link, "CONNECT: Connection to \002%s\002 timed out.", linkID.c_str());
}
...
void TreeSocket::Close()
{
	...
	if (!ConnectionFailureShown)
	{
		...
		SnomaskManager::WriteGlobalSno(Utils->Creator->link, "Connection to '\2%s\2' failed.",linkID.c_str());

		time_t server_uptime = ServerInstance->Time() - this->age;
		if (server_uptime)
		{
			std::string timestr = ModuleSpanningTree::TimeToStr(server_uptime);
			SnomaskManager::WriteGlobalSno(Utils->Creator->link, "Connection to '\2%s\2' was established for %s", linkID.c_str(), timestr.c_str());
		}
	}
}
