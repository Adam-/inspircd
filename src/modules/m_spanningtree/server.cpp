...
CmdResult CommandServer::HandleServer(TreeServer* ParentOfThis, std::vector<std::string>& params)
{
	...
	if (CheckDupe)
	{
		...
		SnomaskManager::WriteToSnoMaskRemote(Utils->Creator->link, "Server \2"+CheckDupe->GetName()+"\2 being introduced from \2" + ParentOfThis->GetName() + "\2 denied, already exists. Closing link with " + ParentOfThis->GetName());
		return CMD_FAILURE;
	}
	...
	if (CheckDupe)
	{
		...
		SnomaskManager::WriteToSnoMaskRemote(Utils->Creator->link, "Server \2"+servername+"\2 being introduced from \2" + ParentOfThis->GetName() + "\2 denied, server ID already exists on the network. Closing link with " + ParentOfThis->GetName());
		return CMD_FAILURE;
	}
	...
	SnomaskManager::WriteToSnoMaskRemote(Utils->Creator->link, "Server \002"+ParentOfThis->GetName()+"\002 introduced server \002"+servername+"\002 ("+description+")");
	return CMD_SUCCESS;
}
...
bool TreeSocket::Outbound_Reply_Server(parameterlist &params)
{
	...
	for (std::vector<reference<Link> >::iterator i = Utils->LinkBlocks.begin(); i < Utils->LinkBlocks.end(); i++)
	{
		...
		{
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Invalid password on link: %s", x->Name.c_str());
			continue;
		}
		...
		if (CheckDupe)
		{
			...
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Server connection from \2"+sname+"\2 denied, already exists on server "+pname);
			return false;
		}
		CheckDupe = Utils->FindServer(sid);
		if (CheckDupe)
		{
			...
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Server \2"+assign(servername)+"\2 being introduced denied, server ID already exists on the network. Closing link.");
			return false;
		}
	...
	SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Server connection from \2"+sname+"\2 denied, invalid link credentials");
	return false;
}
...
bool TreeSocket::CheckDuplicate(const std::string& sname, const std::string& sid)
{
	...
	if (CheckDupe)
	{
		...
		SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Server connection from \2"+sname+"\2 denied, server ID '"+CheckDupe->GetID()+
...
bool TreeSocket::Inbound_Server(parameterlist &params)
{
	...
	for (std::vector<reference<Link> >::iterator i = Utils->LinkBlocks.begin(); i < Utils->LinkBlocks.end(); i++)
	{
		...
		SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Verified incoming server connection " + linkID + " ("+description+")");
