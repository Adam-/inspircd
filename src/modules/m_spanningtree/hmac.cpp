...
bool TreeSocket::ComparePass(const Link& link, const std::string &theirs)
{
	...
	if (capab->auth_fingerprint)
	{
		...
		{
			SnomaskManager::WriteToSnoMask(Utils->Creator->link, "Invalid SSL fingerprint on link %s: need \"%s\" got \"%s\"",
	...
	{
		SnomaskManager::WriteToSnoMask(Utils->Creator->link, "SSL fingerprint for link %s is \"%s\". "
