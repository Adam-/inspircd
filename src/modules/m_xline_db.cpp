 ...
 public:
	...
	bool WriteDatabase()
	{
		...
		if (!stream.is_open())
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: cannot create new db: %s (%d)", strerror(errno), errno);
			return false;
		}
		...
		if (stream.fail())
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: cannot write to new db: %s (%d)", strerror(errno), errno);
			return false;
		}
		stream.close();
		...
		if (remove(xlinedbpath.c_str()))
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: cannot remove old database: %s (%d)", strerror(errno), errno);
			return false;
		}
#endif
		...
		if (rename(xlinenewdbpath.c_str(), xlinedbpath.c_str()) < 0)
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: cannot replace old with new db: %s (%d)", strerror(errno), errno);
			return false;
		}
	...
	bool ReadDatabase()
	{
		...
		if (!stream.is_open())
		{
			...
			SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: cannot read db: %s (%d)", strerror(errno), errno);
			return false;
		}
		...
		while (std::getline(stream, line))
		{
			...
			if (command_p[0] == "VERSION")
			{
				if (command_p[1] != "1")
				{
					...
					SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: I got a database version (%s) I don't understand", command_p[1].c_str());
					return false;
				}
			}
			else if (command_p[0] == "LINE")
			{
				...
				if (!xlf)
				{
					SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "database: Unknown line type (%s).", command_p[1].c_str());
					continue;
				}
				...
				{
					SnomaskManager::WriteToSnoMask(SnomaskManager::xline, "database: Added a line of type %s", command_p[1].c_str());
				}
				else
					delete xl;
			}
