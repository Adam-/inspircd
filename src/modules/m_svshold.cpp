...
public:
	...
	void DisplayExpiry()
	{
		if (!silent)
		{
			SnomaskManager::WriteToSnoMask(SnomaskManager::xline, "Removing expired SVSHOLD %s (set by %s %ld seconds ago)",
 ...
 public:
	...
	CmdResult Handle(const std::vector<std::string> &parameters, User *user)
	{
		...
		if (parameters.size() == 1)
		{
			...
			{
				if (!silent)
					SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s removed SVSHOLD on %s",user->nick.c_str(),parameters[0].c_str());
			}
			else
			{
		...
		else
		{
			...
			if (ServerInstance->XLines->AddLine(r, user))
			{
				...
				if (!duration)
				{
					SnomaskManager::WriteGlobalSno(SnomaskManager::xline, "%s added permanent SVSHOLD for %s: %s", user->nick.c_str(), parameters[0].c_str(), parameters[2].c_str());
				}
				else
				{
					time_t c_requires_crap = duration + ServerInstance->Time();
					std::string timestr = InspIRCd::TimeString(c_requires_crap);
					SnomaskManager::WriteGlobalSno(SnomaskManager::xline, "%s added timed SVSHOLD for %s, expires on %s: %s", user->nick.c_str(), parameters[0].c_str(), timestr.c_str(), parameters[2].c_str());
				}
			}
			else
			{
