	...
	CmdResult Handle(const std::vector<std::string>& parameters, User *user)
	{
		...
		if (IS_LOCAL(target))
		{
			...
			if (target->ForceNickChange(parameters[1]))
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, user->nick+" used NICKLOCK to change and hold "+oldnick+" to "+parameters[1]);
			else
			{
				std::string newnick = target->nick;
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, user->nick+" used NICKLOCK, but "+oldnick+" failed nick change to "+parameters[1]+" and was locked to "+newnick+" instead");
			}
		}
 ...
 public:
	...
	CmdResult Handle (const std::vector<std::string>& parameters, User *user)
	{
		...
		if (IS_LOCAL(target))
		{
			...
			{
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, user->nick+" used NICKUNLOCK on "+target->nick);
