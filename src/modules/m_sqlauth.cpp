...
class AuthQuery : public SQLQuery
{
 public:
	...
	void OnResult(SQLResult& res) CXX11_OVERRIDE
	{
		...
		{
			if (verbose)
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Forbidden connection from %s (SQL query returned no matches)", user->GetFullRealHost().c_str());
	...
	void OnError(SQLerror& error) CXX11_OVERRIDE
	{
		...
		pendingExt.set(user, AUTH_STATE_FAIL);
		if (verbose)
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Forbidden connection from %s (SQL query failed: %s)", user->GetFullRealHost().c_str(), error.Str());
	}
};
...
class ModuleSQLAuth : public Module
{
 ...
 public:
	...
	ModResult OnUserRegister(LocalUser* user) CXX11_OVERRIDE
	{
		...
		if (!SQL)
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Forbiding connection from %s (SQL database not present)", user->GetFullRealHost().c_str());
