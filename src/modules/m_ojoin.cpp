...
class CommandOjoin : public SplitCommand
{
 public:
	...
	CmdResult HandleLocal(const std::vector<std::string>& parameters, LocalUser* user)
	{
		...
		if (channel)
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, user->nick+" used OJOIN to join "+channel->name);
		...
		{
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, user->nick+" used OJOIN in "+parameters[0]);
