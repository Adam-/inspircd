 ...
 public:
	...
	CmdResult Handle(const std::vector<std::string> &parameters, User *user)
	{
		...
		if (parameters.size() == 1)
		{
			...
			{
				SnomaskManager::WriteGlobalSno(SnomaskManager::xline, "%s removed CBan on %s.",user->nick.c_str(),parameters[0].c_str());
			}
			else
			{
		...
		else
		{
			...
			if (ServerInstance->XLines->AddLine(r, user))
			{
				if (!duration)
				{
					SnomaskManager::WriteGlobalSno(SnomaskManager::xline, "%s added permanent CBan for %s: %s", user->nick.c_str(), parameters[0].c_str(), reason);
				}
				else
				{
					time_t c_requires_crap = duration + ServerInstance->Time();
					std::string timestr = InspIRCd::TimeString(c_requires_crap);
					SnomaskManager::WriteGlobalSno(SnomaskManager::xline, "%s added timed CBan for %s, expires on %s: %s", user->nick.c_str(), parameters[0].c_str(), timestr.c_str(), reason);
				}
			}
			else
			{
 ...
 public:
	...
	ModResult OnUserPreJoin(LocalUser* user, Channel* chan, const std::string& cname, std::string& privs, const std::string& keygiven) CXX11_OVERRIDE
	{
		...
		if (rl)
		{
			...
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "%s tried to join %s which is CBANed (%s)",
