 ...
 public:
	...
	void OnLookupComplete(const DNS::Query *r) CXX11_OVERRIDE
	{
		...
		if (match)
		{
			...
			switch (ConfEntry->banaction)
			{
				...
				case DNSBLConfEntry::I_KLINE:
				{
					...
					if (ServerInstance->XLines->AddLine(kl,NULL))
					{
						...
						SnomaskManager::WriteGlobalSno(SnomaskManager::xline,"K:line added due to DNSBL match on *@%s to expire on %s: %s",
				...
				case DNSBLConfEntry::I_GLINE:
				{
					...
					if (ServerInstance->XLines->AddLine(gl,NULL))
					{
						...
						SnomaskManager::WriteGlobalSno(SnomaskManager::xline,"G:line added due to DNSBL match on *@%s to expire on %s: %s",
							them->GetIPString().c_str(), timestr.c_str(), reason.c_str());
						ServerInstance->XLines->ApplyLines();
					}
					else
				...
				case DNSBLConfEntry::I_ZLINE:
				{
					...
					if (ServerInstance->XLines->AddLine(zl,NULL))
					{
						...
						SnomaskManager::WriteGlobalSno(SnomaskManager::xline,"Z:line added due to DNSBL match on *@%s to expire on %s: %s",
							them->GetIPString().c_str(), timestr.c_str(), reason.c_str());
						ServerInstance->XLines->ApplyLines();
					}
					else
			...
			SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Connecting user %s%s detected as being on a DNS blacklist (%s) with result %d", them->nick.empty() ? "<unknown>" : "", them->GetFullRealHost().c_str(), ConfEntry->domain.c_str(), (ConfEntry->type==DNSBLConfEntry::A_BITMASK) ? bitmask : record);
		}
		else
	...
	void ReadConfig(ConfigStatus& status) CXX11_OVERRIDE
	{
		...
		for(ConfigIter i = dnsbls.first; i != dnsbls.second; ++i)
		{
			...
			if ((e->bitmask <= 0) && (DNSBLConfEntry::A_BITMASK == e->type))
			{
				...
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "DNSBL(%s): invalid bitmask", location.c_str());
			}
			else if (e->name.empty())
			{
				std::string location = tag->getTagLocation();
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "DNSBL(%s): Invalid name", location.c_str());
			}
			else if (e->domain.empty())
			{
				std::string location = tag->getTagLocation();
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "DNSBL(%s): Invalid domain", location.c_str());
			}
			else if (e->banaction == DNSBLConfEntry::I_UNKNOWN)
			{
				std::string location = tag->getTagLocation();
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "DNSBL(%s): Invalid banaction", location.c_str());
			}
			else
			{
				if (e->reason.empty())
				{
					std::string location = tag->getTagLocation();
					SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "DNSBL(%s): empty reason, using defaults", location.c_str());
