 ...
 public:
	...
	CmdResult Handle(const std::vector<std::string>& parameters, User *user)
	{
		...
		if (parameters.size() == 1)
		{
			...
			{
				SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s removed SHUN on %s",user->nick.c_str(),target.c_str());
			}
			else
			{
		...
		else
		{
			...
			if (ServerInstance->XLines->AddLine(r, user))
			{
				if (!duration)
				{
					SnomaskManager::WriteToSnoMask(SnomaskManager::xline,"%s added permanent SHUN for %s: %s",
				...
				else
				{
					...
					SnomaskManager::WriteToSnoMask(SnomaskManager::xline, "%s added timed SHUN for %s to expire on %s: %s",
