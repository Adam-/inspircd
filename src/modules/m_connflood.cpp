	...
	ModResult OnUserRegister(LocalUser* user) CXX11_OVERRIDE
	{
		...
		if (throttled)
		{
			if (tdiff > seconds + timeout)
			{
				/* expire throttle */
				throttled = false;
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Connection throttle deactivated");
				return MOD_RES_PASSTHRU;
			}
		...
		if (tdiff <= seconds)
		{
			...
			{
				throttled = true;
				SnomaskManager::WriteGlobalSno(SnomaskManager::announcement, "Connection throttle activated");
				ServerInstance->Users->QuitUser(user, quitmsg);
				return MOD_RES_DENY;
			}
		}
