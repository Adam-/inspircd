...
ModeUserServerNoticeMask::ModeUserServerNoticeMask(const std::string &Name, char c) : ModeHandler(NULL, Name, c, PARAM_ALWAYS, MODETYPE_USER)
{
	oper = true;
	remote = false;
}
...
std::string ModeUserServerNoticeMask::GetUserParameter(User* user)
{
	...
	for (unsigned i = remote; i < user->snomasks.size(); i += 2)
		if (user->snomasks[i])
		{
	...
	for (std::string token; sep.GetToken(token);)
	{
		if (token == "*")
		{
			...
			for (unsigned j = 0; j < v.size(); ++j)
			{
				...
				if (user->snomasks[s->pos + remote] != adding)
				{
					user->snomasks[s->pos + remote] = adding;
