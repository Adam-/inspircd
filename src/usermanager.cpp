...
void UserManager::AddUser(int socket, ListenSocket* via, irc::sockets::sockaddrs* client, irc::sockets::sockaddrs* server)
{
	...
	catch (...)
	{
		...
		SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "WARNING *** Duplicate UUID allocated!");
		return;
	}
	...
	{
		SnomaskManager::WriteToSnoMask(SnomaskManager::announcement, "Warning: softlimit value has been reached: %d clients", ServerInstance->Config->SoftLimit);
...
void UserManager::QuitUser(User* user, const std::string& quitreason, const std::string* operreason)
{
	...
	if (IS_LOCAL(user))
	{
		...
		if (lu->registered == REG_ALL)
			SnomaskManager::WriteToSnoMask(SnomaskManager::quit, "Client exiting: %s (%s) [%s]", user->GetFullRealHost().c_str(), user->GetIPString().c_str(), operreason->c_str());
	}
