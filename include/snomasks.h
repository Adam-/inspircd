...
class CoreExport SnomaskManager
{
 ...
 public:
	 ...
	 */
	static void WriteToSnoMask(Snomask &, const std::string &text);
	/* Same as above, but writes to all local users with the remote version of the snomask */
	static void WriteToSnoMaskRemote(Snomask &, const std::string &text);
	 ...
	 */
	static void WriteToSnoMask(Snomask &snomask, const char* text, ...) CUSTOM_PRINTF(2, 3);
	/* Same as above, but writes to all local users with the remote version of the snomask */
	static void WriteToSnoMaskRemote(Snomask &snomask,  const char* text, ...) CUSTOM_PRINTF(2, 3);

	/** Write to all users with a given snomask (sent globally)
	 * @param letter The snomask letter to write to
	 * @param text The text to send to the users
	 */
	static void WriteGlobalSno(Snomask &, const std::string &text);
	 ...
	 * @param ... Format arguments
	 */
	static void WriteGlobalSno(Snomask &, const char* text, ...) CUSTOM_PRINTF(2, 3);
